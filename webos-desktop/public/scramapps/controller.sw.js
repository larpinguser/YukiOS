...
(() => {
  ...
  ((() => {
          ...
          (this.rpc = new e.C(
            {
              sendSetCookie: async ({ cookies: e, options: r }) => {
                ...
                for (let a of o) {
                  ...
                  (s.push(o),
                    a.postMessage({ $controller$setCookie: { cookies: e, options: r, id: o } }),
                    n ||
                      i.push(
                        new Promise((e) => {
                          t[o] = () => e(o);
                ...
                if (i.length > 0) {
                    ...
                    a = new Promise((i) => {
                      r = setTimeout(() => {
                        if (!n) {
                          ...
                          console.error(
                            `timed out waiting for set cookie response (deadlock?): cookies=${e.length} clients=${o.length} pending=${r.length}/${s.length} clientUrls=${o.map((e) => e.url).join(",")}`
                          );
                        }
                        i();
                      }, 1e3);
                  ...
                  try {
                    ...
                    for (let e of (void 0 !== r && clearTimeout(r), s)) delete t[e];
                  }
                }
              }
            },
      ...
      try {
        let t = new URL(e.request.url),
          ...
          n = await r.rpc.call(
            ...
            e.request.body instanceof ReadableStream || e.request.body instanceof ArrayBuffer
              ? [e.request.body]
              : void 0
          );
        return new Response(n.body, { status: n.status, statusText: n.statusText, headers: n.headers });
      } catch (e) {
        return (
    ...
    (addEventListener("message", (e) => {
      ...
      (-1 !== r && i.splice(r, 1), i.push(new s(t.prefix, t.id, e.ports[0])));
    }),
      ...
      setTimeout(async () => {
        for (let e of (console.log("service worker activated, notifying clients to revive"), await clients.matchAll()))
